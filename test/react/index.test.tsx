...
describe("react", () => {
	...
	it("StrictMode double-mount does not leak subscriptions", () => {
		...
		// StrictMode mounts twice, and cleanup runs between the mounts, so only
		// one subscription is left live.
		assert.equal(liveSubscriptions, 1);
	...
	it("catches a publish that lands before the subscription effect runs", () => {
		...
		// Publisher must come first: sibling effects flush in tree order, so the
		// publish lands before Display subscribes. Swapping them voids the test.
		render(
			<div>
				<Publisher />
				<Display />
